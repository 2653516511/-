...
Note that returning a falsy expression will still cause the element after `&&` to be skipped but will return the falsy expression. In the example below, `<div>0</div>` will be returned by the render method.(???这句话，&&运算符，count为0，判断为false，不执行后面的，{}表达式将false译为0)
...
In rare cases you might want a component to hide itself even though it was rendered by another component. To do this return `null` instead of its render output.（！！好想法）
